...
For shape = 0 To Nshapes - 1
    ...
    g = (shape * 230) Mod 255
    b = 255 - r
    For trail = 0 To Ntrail - 1 ' trail get progressive lower alpha value giving fainter line
        alpha = 255 - alphadropoff + trail / (Ntrail - 1) * alphadropoff
        co(shape, trail) = _RGBA32(r, g, b, alpha)
    Next trail
Next shape

' main animation loop
loopcounter = 0: trail2init = 1
Do
    If trail2init <= Ntrail Then ' initialise trails of all shapes
        ...
        End If
        startstep ' trails still being initialised, only update first shape using dx and dy
    Else
...
Sub report
    ...
    For shape = 0 To Nshapes - 1
        For trail = 0 To Ntrail - 1
            For pnt = 0 To Npoints - 1
                Print x(pnt, shape, trail); y(pnt, shape, trail);
            Next pnt
            Print
        Next trail
    Next shape
