...
Const dxyz = 35 ' distance eyeball to xyz axes origin
Const yzangle = 2 * _Pi / 180 ' angle over which graph is rotated around x axis
...
Dim As Single x, y, z, yr, zr, xscrmax, yscrmax, xscrmin, yscrmin, xscrspan, yscrspan
...
For v = Ny - 1 To 0 Step -1
    ...
    For h = 0 To Nx - 1
        ...
        co(v, h) = 4 + (z - zl) / (zu - zl) * 255 + (1 - v / Ny) * 150
        ' rotate 3D coord. around x axis to look tilt graph for viewing
        yr = xrotate!(y, z, yzangle)
        zr = yrotate!(y, z, yzangle)
        ' calculate and store projected coordinates
        ' keep track of max and min values
        points(v, h).x = xproj!(x, yr)
        If points(v, h).x > xscrmax Then xscrmax = points(v, h).x
        If points(v, h).x < xscrmin Then xscrmin = points(v, h).x
        points(v, h).y = yproj!(yr, zr)
